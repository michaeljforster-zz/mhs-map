...
(hunchentoot:define-easy-handler (handle-admin-login
  ...
  (sli-hunchentools:harden-session-cookie)
  ...
  (if (equal (hunchentoot:request-method*) :post)
      (if (and (string= username app-config:*username*)
          ...
          (progn
            (setf (sli-hunchentools:session-user) (make-admin-user :username username))
            (sli-hunchentools:delete-session-csrf-token)
            (sli-hunchentools:session-csrf-token)
...
(hunchentoot:define-easy-handler (handle-admin-logout
  ...
  (hunchentoot:no-cache)
  (sli-hunchentools:delete-session-csrf-token)
  (sli-hunchentools:delete-session-user)
